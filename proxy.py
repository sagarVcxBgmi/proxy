...
# Load proxies from the file and select one at random
proxies_list = load_proxies("Webshare 10 proxies.txt")
    ...
    with open("user_access_backup.txt", "w") as backup_file:
        for uid, exp in user_access.items():
            try:
                ...
                logging.error(f"Error retrieving chat info for {uid}